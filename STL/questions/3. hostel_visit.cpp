...
long long int rocket(long long int x,long long int y) 
{
    return x*x+y*y;
}

void visit(priority_queue<long long int> &pq, int k)
{
    while(pq.size()>k)
    {
        pq.pop();
...
void display(priority_queue<long long int> pq)
{
    priority_queue<long long int >q=pq;
    while(!q.empty())
    {
        cout<<q.top()<<"\t";
        q.pop();
